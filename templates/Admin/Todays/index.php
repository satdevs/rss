 ...
 * @var \App\Model\Entity\Today[]|\Cake\Collection\CollectionInterface $todays
 */
?>
<?php 
...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Todays') ?>	
				<div class="card-body table-responsive p-0 todays">
...
<?php } ?>
					...
					<?php foreach ($todays as $today): ?>
							<tr row-id="<?= $today->id ?>"<?php if($today->id == $layoutTodaysLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $today->id ?>"><a class="anchor" name="<?= $today->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($today->id) ?>"><?= $this->Number->format($today->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="year integer text-center" name="year" value="<?= $this->Number->format($today->year) ?>"><?= $this->Number->format($today->year) ?></td><!-- bake-3 -->
								<td class="month integer text-center" name="month" value="<?= $this->Number->format($today->month) ?>"><?= $this->Number->format($today->month) ?></td><!-- bake-3 -->
								<td class="day integer text-center" name="day" value="<?= $this->Number->format($today->day) ?>"><?= $this->Number->format($today->day) ?></td><!-- bake-3 -->
								<td class="title string" name="title" value="<?= $today->title ?>"><?= h($today->title) ?></td><!-- bake-2 -->
								<td class="counter integer text-center" name="counter" value="<?= $this->Number->format($today->counter) ?>"><?= $this->Number->format($today->counter) ?></td><!-- bake-3 -->
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $today->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $today->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if(isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>					  
									<?php //= $this->Form->postLink('<i class="fas fa-remove"></i>', ['action' => 'delete', $today->id], ['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $today->id), 'class' => 'btn btn-sm bg-gradient-danger action-button-delete']) ?>						
									<?= $this->Form->postLink('', ['action' => 'delete', $today->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($today->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Todays', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/todays/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/todays?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/todays?sort=", "<?= $base ?>/<?= $prefix ?>/todays?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/todays']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/todays", "<?= $base ?>/<?= $prefix ?>/todays?page=1");
		});
<?php } ?>
