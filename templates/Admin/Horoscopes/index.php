 ...
 * @var \App\Model\Entity\Horoscope[]|\Cake\Collection\CollectionInterface $horoscopes
 */
?>
<?php 
	...
	if(!isset($layoutHoroscopesLastId)){
		$layoutHoroscopesLastId = 0;
	}
...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Horoscopes') ?>	
				<div class="card-body table-responsive p-0 horoscopes">
...
<?php } ?>
								...
								<th class="today date"><?= $this->Paginator->sort('today') ?></th>
...
<?php } ?>
					...
					<?php foreach ($horoscopes as $horoscope): ?>
							<tr row-id="<?= $horoscope->id ?>"<?php if($horoscope->id == $layoutHoroscopesLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $horoscope->id ?>"><a class="anchor" name="<?= $horoscope->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($horoscope->id) ?>"><?= $this->Number->format($horoscope->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $horoscope->name ?>"><?= h($horoscope->name) ?></td><!-- bake-2 -->
								<td class="today date" name="today" value="<?= $horoscope->today ?>"><?= h($horoscope->today) ?></td><!-- bake-2 -->
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $horoscope->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $horoscope->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if(isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>					  
									<?php //= $this->Form->postLink('<i class="fas fa-remove"></i>', ['action' => 'delete', $horoscope->id], ['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $horoscope->id), 'class' => 'btn btn-sm bg-gradient-danger action-button-delete']) ?>						
									<?= $this->Form->postLink('', ['action' => 'delete', $horoscope->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($horoscope->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Horoscopes', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/horoscopes/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/horoscopes?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/horoscopes?sort=", "<?= $base ?>/<?= $prefix ?>/horoscopes?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/horoscopes']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/horoscopes", "<?= $base ?>/<?= $prefix ?>/horoscopes?page=1");
		});
<?php } ?>
